...
;(function(factory) {
	...
	Slick.prototype.getNavigableIndexes = function () {
		...
		// Non-infinite → clean and simple
		if (!_.options.infinite) {
			return Array.from(
				{ length: Math.ceil(slideCount / slidesToScroll) },
				(_, i) => i * slidesToScroll
			);
		}

		// Infinite → fixed step (always slidesToScroll!)
		const start = -slideCount + 1;
		const end   = slideCount * 2;

		const totalSteps = Math.ceil((end - start) / slidesToScroll);

		return Array.from({ length: totalSteps }, (_, i) => start + (i * slidesToScroll))
					.filter(i => i < end);
	};
...
jQuery( function( $ ) {
	...
	Codevz_Plus.slick = function( wpb, slick ) {
		...
		// Single pages related items carousel.
		if ( body.hasClass( 'single' ) ) {

			$( '.cz_related_posts_inner, .related.products, .codevz-recently-viewed-products' ).each( function() {

				var $this = $( this );

				if ( ! $this.hasClass( 'codevz-related-slick' ) ) {

					var slidesToShow = $this.attr( 'data-col' ),
						slidesToShow = slidesToShow ? slidesToShow : parseInt( $this.find( 'ul.products' ).attr( 'class' ).match( /columns-(\d+)/)?.[1] || 3 );

					$this.addClass( 'codevz-plus-related-carousel no_dots' );
					$this.attr( 'data-slick-prev', 'fa fa-chevron-left' );
					$this.attr( 'data-slick-next', 'fa fa-chevron-right' );
					$this.attr( 'data-slick', '{"selector":"","slidesToShow":' + slidesToShow + ',"slidesToScroll":1,"infinite":true,"speed":1000,"swipeToSlide":true,"autoplay":false,"dots":false,"counts":false,"responsive":[{"breakpoint":769,"settings":{"slidesToShow":2,"slidesToScroll":1,"infinite":true,"touchMove":true}},{"breakpoint":481,"settings":{"slidesToShow":1,"slidesToScroll":1,"infinite":true,"touchMove":true}}]}' );

					$this.addClass( 'codevz-related-slick' );
		...
		$( '[data-slick]' ).not( '.cz_popup_modal:not(.cz_show_popup) [data-slick]' ).codevzPlus( 'slick', function( x ) {
			...
			var opt = x.data( 'slick' ),
				selector = function( selector ) {
					...
					} else if ( $( '.brand-thumbnails', selector ).length ) {
						return $( '.brand-thumbnails', selector );
					} else {
						return selector;
					}
				},
			...
			// Remove first level cz_gap.
			dis.find( '> .cz_gap' ).remove();
			...
			setTimeout( function() {
				...
				// Onload.
				dis.addClass( 'slick-no-loading' );

				// Fix clip-path.
				const list = dis.find( '.slick-list' );
				const clipPath = dis.find( '.slick-list' ).css( 'clip-path' );
				if ( clipPath ) {
					list.css( 'clip-path', clipPath.replace(/\d+/g, n => Math.max(0, parseInt(n) - 1)) );
				}

				// Fix arrows for related posts.
				if ( body.hasClass( 'single' ) && dis.closest( '.codevz-plus-related-carousel' ).length && ! dis.closest( '.codevz-plus-related-carousel' ).find( '.codevz-plus-related-arrows' ).length ) {

					setTimeout( function() {

						var headline = dis.closest( '.codevz-plus-related-carousel' ).find( '> h1, > h2, > h3, > h4' );

						if ( dis.hasClass( 'cz_related_posts_inner' ) ) {
							headline = dis.parent().find( '> h1, > h2, > h3, > h4' );
						}

						$( '<div class="codevz-plus-related-arrows" />' ).append( dis.find( '.slick-arrow' ).removeClass( 'slick-arrow' ) ).appendTo( headline );
				...
				// Panorama 3D effect type.
				if ( dis.closest( '.codevz-plus-carousel-panorama' ).length ) {

					function updatePanoramaEffect( slider, slidesToShow = 3, currentSlide = 0 ) {
					  ...
					  slider.find('.slick-slide').each( function() {
						...
						const $slide = $( this );
						let offset = $slide.data( 'slick-index' ) - currentSlide;

						if (offset > totalSlides / 2) { offset -= totalSlides; }
						if (offset < -totalSlides / 2) { offset += totalSlides; }

						let one = slidesToShow == 1;
						let five = slidesToShow == 5;
						let seven = slidesToShow == 7;

						const transforms = {
							'-4': 'rotateY(' + ( five ? 65 : ( seven ? 59 : 90 ) ) + 'deg) scale(' + ( five ? '2.3' : ( seven ? '1.97' : '3.1' ) ) + ')',
							'-3': 'rotateY(' + ( five ? 65 : ( seven ? 49 : 90 ) ) + 'deg) scale(' + ( five ? '2.3' : ( seven ? '1.5' : '3.1' ) ) + ')',
							'-2': 'rotateY(' + ( five ? 49 : ( seven ? 38 : 61 ) ) + 'deg) scale(' + ( five ? '1.5' : ( seven ? '1.26' : '2.15' ) ) + ')',
							'-1': 'rotateY(' + ( seven ? 24 : 33 ) + 'deg) scale(1.' + ( five ? 18 : ( seven ? 1 : 26 ) ) + ')',
							'0': 'rotateY(' + ( five ? 16 : 10 ) + 'deg) scale(1.' + ( seven ? '02' : '04' ) + ')',
							'1': 'rotateY(0) scale(1)',
							'2': 'rotateY(-' + ( five ? 16 : 10 ) + 'deg) scale(1.' + ( seven ? '02' : '04' ) + ')',
							'3': 'rotateY(-' + ( seven ? 24 : 33 ) + 'deg) scale(1.' + ( five ? 18 : ( seven ? 1 : 26 ) ) + ')',
							'4': 'rotateY(-' + ( five ? 49 : ( seven ? 38 : 61 ) ) + 'deg) scale(' + ( five ? '1.5' : ( seven ? '1.26' : '2.15' ) ) + ')',
							'5': 'rotateY(-' + ( five ? 65 : ( seven ? 49 : 90 ) ) + 'deg) scale(' + ( five ? '2.3' : ( seven ? '1.5' : '3.1' ) ) + ')',
							'6': 'rotateY(-' + ( five ? 65 : ( seven ? 59 : 90 ) ) + 'deg) scale(' + ( five ? '2.3' : ( seven ? '1.97' : '3.1' ) ) + ')'
						};

						offset = one ? offset + 2 : five ? offset - 1 : seven ? offset - 2 : offset;

						const transform = transforms[ offset ];
						$slide.css({ transform: 'perspective(1000px) ' + transform ?? 'none' });

					  });

					}

					// Panorama 3D on change.
					dis.on( 'init reInit beforeChange afterChange', function ( event, slick, current, next ) {
						const slideIndex = next ?? slick.slickCurrentSlide();
						updatePanoramaEffect( dis, slick.options.slidesToShow, slideIndex );
					});

					// Panorama 3D on swipe.
					const slickInstance = dis.slick( 'getSlick' );
					const originalSwipeMove = slickInstance.swipeMove;
					slickInstance.swipeMove = function( event ) {
						const result = originalSwipeMove.call( this, event );
						if ( this.dragging && typeof this.swipeLeft === 'number' ) {
							const slideWidth = this.slideWidth;
							const approxSlide = Math.round( -this.swipeLeft / slideWidth );
							updatePanoramaEffect( dis, this.options.slidesToShow, approxSlide - 3);
						}
						return result;
					};
			...
			// Refresh for VC & Elementor live changes.
			if ( wpb || $( '.elementor-editor-active' ).length ) {
			...
			// Fix carousel inside WPBakery tabs.
	...
	// Enable keyboard left/right on active carousel.
	document.addEventListener('keydown', (e) => {
	    if ( e.keyCode === 37 || e.keyCode === 39 ) {
	        document.querySelectorAll('[data-slick]').forEach(slider => {
	            const r = slider.getBoundingClientRect();
	            if (r.top <= window.innerHeight && r.bottom >= 0) {
	                const btn = slider.querySelector(`.slick-${e.keyCode === 37 ? 'prev' : 'next'}`);
	                if (btn) btn.click();
	            }
	        });
	    }
