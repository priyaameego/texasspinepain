...
var Codevz_Plus = ( function( $ ) {
	...
	return {
		...
		init: function() {
			...
			// Top banner cookie.
			if ( document.querySelector( '.codevz-top-banner-always' ) ) {
		...
		responsiveClamp: function() {
			...
			if ( document.body.classList.contains( 'elementor-editor-active' ) || document.body.classList.contains( 'vc_editor' ) ) {
				return;
			}

			var widgets_shape = document.querySelectorAll('.codevz-widget-title');

			// Only mobile.
			if ( window.innerWidth > 768 ) {

				if ( widgets_shape.length ) {
					widgets_shape.forEach( el => el.classList.remove( 'codevz-widget-title-mobfix-shape' ) );
				}

				return;
			}

			// Fix widget title shape center mode.
			if ( widgets_shape.length ) {
			    widgets_shape.forEach( el => {
			        const s = getComputedStyle( el );
			        let w = s.borderBottomWidth;

			        if ( s.borderBottomStyle === 'none' && parseFloat( w ) === 0 ) {
			            const o = el.style.borderBottomStyle;
			            const p = el.style.getPropertyPriority( 'border-bottom-style' );

			            el.style.setProperty( 'border-bottom-style', 'solid', 'important' );
			            w = getComputedStyle( el ).borderBottomWidth;
			            el.style.setProperty( 'border-bottom-style', o, p );
			        }
			        
			        if ( s.textAlign === 'center' && parseFloat( w ) > 0 ) {
			            el.classList.add( 'codevz-widget-title-mobfix-shape' );
			        }
			    });
			}
			...
			setTimeout( function() {
				...
				$( '.cz_sticky .row > aside, .cz_sticky_col, .column-xtra-sticky, .woocommerce-Reviews #review_form_wrapper' ).codevzPlus( 'sticky-col', function( x ) {
					...
					x.theiaStickySidebar(
						{
							additionalMarginTop: ( stickyHeader.length ? stickyHeader.outerHeight() : 0 ) + 60,
